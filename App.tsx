...
// Split Modal States
  ...
  const handleExportCSV = () => {
    ...
    // Add UTF-8 BOM (\uFEFF) so Excel reads special characters (ľ, š, č, ť, ž) correctly
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-5 pt-8">
        ...
        <div className="flex gap-2 mb-8 bg-slate-100/50 p-1.5 rounded-3xl sticky top-20 z-20 backdrop-blur-md">
           <button 
             ...
             className={`flex-1 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${
               activeTab === 'list' ? 'bg-white shadow-md text-indigo-600' : 'text-slate-400 hover:text-slate-600'
             }`}
           >
             <Package size={14} /> {t.gear}
           </button>
           <button 
             onClick={() => setActiveTab('stats')}
             className={`flex-1 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${
               activeTab === 'stats' ? 'bg-white shadow-md text-indigo-600' : 'text-slate-400 hover:text-slate-600'
             }`}
           >
